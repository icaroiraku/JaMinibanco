...
public class ContaPoupancaService {
	...
	public BigDecimal deposita (ContaPoupanca contaPoupanca,BigDecimal valor) throws Exception {
		
		ContaPoupanca contaPoupancaMemoria = contasPoupancas.get(contaPoupanca.getNumeroConta());
		if(contaPoupancaMemoria ==  null) {
			
			throw new Exception("Conta Poupança não encontrada");
		}
		return contaPoupancaMemoria.getSaldoDisponivel().add(valor);
	}
